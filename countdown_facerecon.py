...
def render_custom_message(message, font_size = font_size):
    """Función para renderizar un mensaje personalizado en la pantalla."""
    # Limpia la pantalla
    screen.fill((0, 0, 0))  # Fondo negro
    
    # Configura el color y la fuente del texto
    font = pygame.font.Font(None,font_size)  # Tamaño de fuente de ejemplo
    text_surface = font.render(message, True, COLORS["red"])  # Texto en blanco
    
    # Posiciona el texto en el centro de la pantalla
...
file_path = 'name_message.txt'
while running:
    # Dentro del bucle while principal
    if os.path.exists(file_path):
        ...
        with open(file_path, 'r') as file:
            name = file.read()  # Leer el contenido del archivo
            message = message_id[name.strip()]
        render_custom_message(message, font_size=70) 
